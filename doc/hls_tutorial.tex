...
\documentclass[Chinese,TC,use boldface,simple name]{beaulivre}
...
\newtcbox{\UGvitis}{enhanced,nobeforeafter,tcbox raise base,boxrule=0.4pt,top=0mm,bottom=0mm,
  ...
  overlay={\begin{tcbclipinterior}\fill[green!75!blue!50!white] (frame.south west)
    rectangle node[text=white,font=\sffamily\small,rotate=90] {UG} ([xshift=4mm]frame.north west);\end{tcbclipinterior}}}
...
\TitlePage [
  ...
  ]
  {
    ...
    , subtitle  = {
                    ...
                    \tiny Xilinx Vivado \texttt{2017.4}\\[-8pt]
                    \tiny Xilinx Vitis\hspace{1.65mm} \texttt{2021.2}
                  }
    , author    = 赵舞穹，LEADS
    , date      = {\today{}，南京}
  }
...
\chapter{FFT}\index{FFT}
  ...
  \section{FFT Naive}
    ...
    \subsection{代码实现}
      ...
      \begin{definition}[整数类型]\index{Zzhengshuleixing@整数类型}
        整数（integer）类型包括有符号（signed）和无符号（unsigned）两种。
        除去使用 C++ 中定义的 \texttt{char}、\texttt{short}、\texttt{int}、\texttt{long}、\texttt{long long} 及其 \texttt{unsigned} 版本变量外，建议在涉及硬件实现的位数时使用 HLS 工具提供的 \indextt{ap\_int}\texttt{ap\_int} 和 \indextt{ap\_uint}\texttt{ap\_uint}。
        这些类型可以自由定义其位数，详见 UG 第 80 和 218 页。
      \end{definition}
      \begin{example}[整数类型]
...
TYPE_T d;                 // 6-bit signed value
        ...
        另外，默认的最大宽度是1024比特，如果需要更宽的数值（例如 4096），需要在 \verb|#include<ap_int.h>| 之前添加\indextt{AP\_INT\_MAX\_W}
        \begin{lstlisting}[language=C++,numbers=none]
